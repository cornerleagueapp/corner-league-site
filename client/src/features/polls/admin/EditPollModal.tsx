...
import { usePollById, useUpdatePoll } from "../hooks/usePolls";
...
const votingModeOptions: PollVotingMode[] = ["fixed_options", "racer_search"];
...
export function EditPollModal({ poll, open, onClose }: EditPollModalProps) {
  const updatePoll = useUpdatePoll();

  const { data: fullPoll, isLoading: isPollLoading } = usePollById(
    open && poll?.id ? poll.id : undefined,
  );
  ...
  useEffect(() => {
    const source = fullPoll ?? poll;

    if (!source) return;

    setTitle(source.title ?? "");
    setDescription(source.description ?? "");
    setCategory(source.category ?? "fan_vote");
    setVotingMode(source.votingMode ?? "fixed_options");
    setStatus(source.status ?? "draft");
    setIsFeatured(Boolean(source.isFeatured));
    setIsPublic(Boolean(source.isPublic));
    setCoverImageUrl(source.coverImageUrl ?? "");
    setRewardTitle(fullPoll?.rewardTitle ?? "");
    setRewardDescription(fullPoll?.rewardDescription ?? "");
    setEndsAt(toDatetimeLocal(source.endsAt));

    if (fullPoll?.options?.length) {
      setOptions(
        fullPoll.options.map((option) => ({
          label: option.label ?? "",
    ...
    } else if ((source.votingMode ?? "fixed_options") === "fixed_options") {
      setOptions([
        { label: "", description: "", imageUrl: "" },
        { label: "", description: "", imageUrl: "" },
      ]);
  ...
  function addOption() {
    setOptions((current) => [
      ...current,
      { label: "", description: "", imageUrl: "", racerId: null },
    ]);
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (votingMode === "fixed_options" && cleanOptions.length < 2) {
        setError("Fixed option polls need at least two options.");
        return;
      }
  ...
  return (
    <>
      <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/75 p-0 backdrop-blur-sm sm:items-center sm:p-4">
        <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-t-[30px] border border-cyan-300/10 bg-[#07111F] shadow-2xl sm:rounded-[30px]">
          ...
          <div className="overflow-y-auto px-5 py-5">
            ...
            {isPollLoading ? (
              <div className="mb-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm text-white/50">
                Loading full poll details...
              </div>
            ) : null}
            ...
            {votingMode === "fixed_options" ? (
              ...
              <div className="mt-6 rounded-[20px] border border-cyan-300/10 bg-cyan-300/[0.04] p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-white/55">
                  Users will search and select any racer as their vote. You do
                  not need to create fixed options for this poll.
                </p>
              </div>
            )}
          </div>
