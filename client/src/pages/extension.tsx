...
export default function Extension() {
  ...
  const [deleteConfirmation, setDeleteConfirmation] = useState<{show: boolean, clubId: string, clubName: string}>({
    show: false,
    clubId: '',
    clubName: ''
  });
  ...
  useEffect(() => {
    ...
    if (storedClub) {
      ...
      setSettingsForm({
        ...
        streamingSource: data.streamingSource === "espn" ? "youtube" : (data.streamingSource || "nfl"),
        ...
        fetch(`/api/clubs/${data.id}`)
          ...
          .then(freshData => {
            ...
            setSettingsForm({
              ...
              streamingSource: updatedClubData.streamingSource === "espn" ? "youtube" : (updatedClubData.streamingSource || "nfl"),
    ...
    if (clubData?.id && user) {
      ...
      if (user.id === clubData.ownerId) {
        fetch(`/api/clubs/${clubData.id}/members`)
          ...
          .catch(error => console.error('Failed to load club members:', error));
      }
    }
  }, [clubData?.id, user]);
  ...
  const updateClubMutation = useMutation({
    ...
    onError: (error) => {
      ...
      toast({
        title: "Update failed",
        description: error.message || "Failed to update club settings. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const shareText = `Join my ${clubData?.clubName || 'sports'} club on Corner League! ${shareUrl}`;
  ...
  const fetchOEmbed = async (url: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Embed Failed",
          description: "Unable to embed this URL. Please check if it's a valid YouTube video.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleMemberMute = async (memberId: string, shouldMute: boolean) => {
    try {
      const response = await apiRequest("POST", `/api/clubs/${clubData.id}/members/${memberId}/mute`, {
        isMuted: shouldMute
      });
      ...
      if (response.ok) {
        ...
        setClubMembers(prev => prev.map(member => 
          member.userId === memberId ? { ...member, isMuted: shouldMute } : member
        ));
        
        toast({
          title: shouldMute ? "Member muted" : "Member unmuted",
          description: shouldMute ? "Member cannot post messages or embed content" : "Member can now post messages and embed content",
        });
      }
    } catch (error) {
      toast({
  ...
  const handleMemberRemove = async (memberId: string) => {
    if (!confirm("Are you sure you want to remove this member? They will be banned from rejoining the club.")) {
      return;
    }
    
    try {
      const response = await apiRequest("POST", `/api/clubs/${clubData.id}/members/${memberId}/remove`, {});
      
      if (response.ok) {
        // Remove member from local state
        setClubMembers(prev => prev.filter(member => member.userId !== memberId));
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4 min-w-0 flex-1">
            ...
            {clubData?.buyMeCoffeeUsername && (
              <button
                onClick={() => window.open(`https://buymeacoffee.com/${clubData.buyMeCoffeeUsername}`, '_blank')}
        ...
        <div className="flex justify-center mb-6">
          <div className="flex items-center gap-3">
            <label htmlFor="zoom-select" className="text-sm font-medium text-gray-300">
              Zoom:
            </label>
            <select
              id="zoom-select"
        ...
        <div className="w-full h-96 bg-gray-900 rounded-lg overflow-hidden border border-gray-700">
          ...
          >
            <iframe
              ...
              src={
                clubData?.streamingSource === "youtube" ? "https://www.fubo.tv/welcome" :
                "https://www.nfl.com/network/watch/nfl-network-live"
              }
              title={
                clubData?.streamingSource === "youtube" ? "Fubo TV" :
                "NFL Network Live"
              }
              frameBorder="0"
        ...
        <div className="mt-8 w-full">
          <div className="px-4 py-3 mb-4">
            <h3 className="text-lg font-semibold text-white opacity-90">Live Chat</h3>
          </div>
          
          {/* Messages Area - Floating Style */}
          <div className="h-64 overflow-y-auto px-4 space-y-4">
            {messages.map((message) => {
              const isCurrentUser = user && message.user === (user.firstName || user.username);
              return (
                <div key={message.id} className="flex flex-col">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-sm font-medium ${isCurrentUser ? 'text-blue-400' : 'text-green-400'}`}>
                      {isCurrentUser ? 'You' : message.user}
                    </span>
                    <span className="text-xs text-gray-500 opacity-70">{message.timestamp}</span>
                  </div>
                  <div className={`max-w-fit ${isCurrentUser ? 'ml-auto' : ''}`}>
                    <p className={`text-sm px-4 py-2 rounded-full backdrop-blur-sm ${
                      isCurrentUser 
        ...
        <div className="mt-8 w-full max-w-2xl mx-auto">
          <div className="bg-gray-900 rounded-lg border border-gray-700 p-6">
            <h3 className="text-lg font-semibold text-white mb-4">Share Highlights</h3>
            <p className="text-sm text-gray-400 mb-4">Post some dope sports videos on Youtube. Just paste the Youtube URL.</p>
            ...
            <form onSubmit={handleEmbedSubmit} className="mb-4">
              <div className="flex gap-3">
                <input
                  ...
                  placeholder={isMuted ? "You are muted and cannot embed content" : "Enter YouTube video URL (e.g., https://www.youtube.com/watch?v=dQw4w9WgXcQ)"}
              ...
              {isMuted && (
                <p className="text-sm text-red-400 mt-2">
                  You have been muted by the club owner and cannot embed content.
                </p>
              )}
            </form>
            ...
            {embedHtml && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-md font-medium text-white">Embedded Content</h4>
                  <button
                    onClick={clearEmbed}
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-lg w-full max-w-md">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2">Share this link to invite others to your club</p>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-6 z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-lg w-full max-w-md max-h-[90vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1">
              ...
              {/* Club Name */}
              <div>
                <label htmlFor="settingsClubName" className="block text-sm font-medium text-gray-300 mb-2">
                  Club Name
                </label>
                <input
                  type="text"
                  ...
                  onChange={(e) => handleSettingsChange("clubName", e.target.value)}
              ...
              {/* Club Description */}
              <div>
                <label htmlFor="settingsDescription" className="block text-sm font-medium text-gray-300 mb-2">
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Let members know what your club is about</p>
              </div>

              {/* Member Capacity */}
              <div>
                <label htmlFor="settingsMaxMembers" className="block text-sm font-medium text-gray-300 mb-2">
                  Maximum Members
                </label>
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => handleSettingsChange("maxMembers", parseInt(e.target.value) || 1)}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleSettingsChange("streamingSource", e.target.value)}
                  className="w-full px-4 py-3 bg-gray-800 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                ...
                <div className="text-sm text-gray-400 mt-2">
                  {settingsForm.streamingSource === "nfl" && "Stream live NFL content and highlights"}
                  {settingsForm.streamingSource === "youtube" && "Stream live TV and sports on Fubo TV"}
                </div>
              </div>
                ...
                <div className="space-y-3">
                  <label className="flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={() => handleSettingsChange("isPrivate", false)}
                    ...
                    <div className="ml-3">
                      ...
                      <div className="text-sm text-gray-400">Anyone can discover and join your club</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center cursor-pointer">
                    ...
                    <div className="ml-3">
                      ...
                      <div className="text-sm text-gray-400">Only people with an invite can join</div>
                    </div>
                  </label>
                </div>
              </div>
              ...
              {/* Buy Me a Coffee */}
              <div>
                <label htmlFor="settingsBuyMeCoffee" className="block text-sm font-medium text-gray-300 mb-2">Tip (Optional)</label>
                <input
                  type="text"
                  id="settingsBuyMeCoffee"
                  value={settingsForm.buyMeCoffeeUsername}
                  onChange={(e) => handleSettingsChange("buyMeCoffeeUsername", e.target.value)}
                  placeholder="your-url"
                  className="w-full px-4 py-3 bg-gray-800 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
                <p className="text-xs text-gray-500 mt-1">Members can tip. Just go to buymeacoffee.com, create an account, and input your url.</p>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-700">
                ...
                <button
                  type="submit"
                  disabled={!settingsForm.clubName.trim() || updateClubMutation.isPending}
              ...
              <div className="pt-4 border-t border-gray-700">
                ...
                {clubMembers.length === 0 ? (
                  <p className="text-gray-400 text-sm">No members have joined this club yet.</p>
                ) : (
                  <div className="space-y-3 max-h-48 overflow-y-auto">
                    {clubMembers.map((member) => (
                      <div key={member.userId} className="flex items-center justify-between p-3 bg-gray-800 rounded-md">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-gray-400 text-xs">@{member.user.username}</p>
                          </div>
                          {member.isMuted && (
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleMemberMute(member.userId, !member.isMuted)}
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Muted members cannot send messages or embed content. Removed members are banned and cannot rejoin.
                </p>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-lg border border-gray-700 p-6 w-full max-w-md">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white">Delete Club</h3>
                <p className="text-gray-400 text-sm">This action cannot be undone</p>
              </div>
            </div>
            
            <p className="text-gray-300 mb-6">
              Are you sure you want to delete <span className="font-semibold text-white">"{deleteConfirmation.clubName}"</span>? 
              This will permanently remove the club and all its data including chat history.
            </p>
            
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteConfirmation({ show: false, clubId: '', clubName: '' })}
